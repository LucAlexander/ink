...
deep_copy_expr_type_replace_prevent_recursion(walker* const walk, expr_ast* source, clash_relation* const relation, token* const rec_name, type_ast* const rec_type){
	...
	return deep_copy_expr_type_replace_worker(walk, source, relation, &realias, rec_name, rec_type, 0);
}

expr_ast*
deep_copy_expr_type_replace_worker(walker* const walk, expr_ast* source, clash_relation* const relation, token_map* const realias, token* const rec_name, type_ast* const rec_type, uint8_t access){
	...
	*new = *source;
	...
	case LAMBDA_EXPR:
		...
		new->data.lambda.expression = deep_copy_expr_type_replace_worker(walk, source->data.lambda.expression, relation, realias, rec_name, rec_type, 0);
		if (new->data.lambda.alt != NULL){
			new->data.lambda.alt = deep_copy_expr_type_replace_worker(walk, source->data.lambda.alt, relation, realias, rec_name, rec_type, 0);
		}
		return new;
	case BLOCK_EXPR:
		...
		for (uint64_t i = 0;i<new->data.block.line_count;++i){
			new->data.block.lines[i] = *deep_copy_expr_type_replace_worker(walk, &source->data.block.lines[i], relation, realias, rec_name, rec_type, 0);
		}
		return new;
	case LIT_EXPR:
		return new;
	case TERM_EXPR:
		...
		*new->data.term = *source->data.term;
		...
		new->data.term->expression = deep_copy_expr_type_replace_worker(walk, source->data.term->expression, relation, realias, rec_name, rec_type, 0);
		return new;
	case STRING_EXPR:
		return new;
	case LIST_EXPR:
		...
		for (uint64_t i = 0;i<new->data.list.line_count;++i){
			new->data.list.lines[i] = *deep_copy_expr_type_replace_worker(walk, &source->data.list.lines[i], relation, realias, rec_name, rec_type, 0);
		}
		return new;
	case STRUCT_EXPR:
		...
		for (uint64_t i = 0;i<new->data.constructor.member_count;++i){
			new->data.constructor.members[i] = *deep_copy_expr_type_replace_worker(walk, &source->data.constructor.members[i], relation, realias, rec_name, rec_type, 0);
		}
		return new;
	case BINDING_EXPR:
		if (access == 1){
		...
		return new;
	case MUTATION_EXPR:
		new->data.mutation.left = deep_copy_expr_type_replace_worker(walk, source->data.mutation.left, relation, realias, rec_name, rec_type, 0);
		new->data.mutation.right = deep_copy_expr_type_replace_worker(walk, source->data.mutation.right, relation, realias, rec_name, rec_type, 0);
		return new;
	case RETURN_EXPR:
		new->data.ret = deep_copy_expr_type_replace_worker(walk, source->data.ret, relation, realias, rec_name, rec_type, 0);
		return new;
	case SIZEOF_EXPR:
		new->data.size_type = deep_copy_type_replace(walk->parse->mem, relation, source->data.size_type);
		return new;
	case REF_EXPR:
		new->data.ref = deep_copy_expr_type_replace_worker(walk, source->data.ref, relation, realias, rec_name, rec_type, 0);
		return new;
	case DEREF_EXPR:
		new->data.deref = deep_copy_expr_type_replace_worker(walk, source->data.deref, relation, realias, rec_name, rec_type, 0);
		return new;
	case IF_EXPR:
		new->data.if_statement.pred = deep_copy_expr_type_replace_worker(walk, source->data.if_statement.pred, relation, realias, rec_name, rec_type, 0);
		new->data.if_statement.cons = deep_copy_expr_type_replace_worker(walk, source->data.if_statement.cons, relation, realias, rec_name, rec_type, 0);
		if (new->data.if_statement.alt != NULL){
			new->data.if_statement.alt = deep_copy_expr_type_replace_worker(walk, source->data.if_statement.alt, relation, realias, rec_name, rec_type, 0);
		}
		return new;
	case FOR_EXPR:
		new->data.for_statement.initial = deep_copy_expr_type_replace_worker(walk, source->data.for_statement.initial, relation, realias, rec_name, rec_type, 0);
		new->data.for_statement.limit = deep_copy_expr_type_replace_worker(walk, source->data.for_statement.limit, relation, realias, rec_name, rec_type, 0);
		new->data.for_statement.change = deep_copy_expr_type_replace_worker(walk, source->data.for_statement.change, relation, realias, rec_name, rec_type, 0);
		new->data.for_statement.cons = deep_copy_expr_type_replace_worker(walk, source->data.for_statement.cons, relation, realias, rec_name, rec_type, 0);
		return new;
	case WHILE_EXPR:
		new->data.while_statement.pred = deep_copy_expr_type_replace_worker(walk, source->data.while_statement.pred, relation, realias, rec_name, rec_type, 0);
		new->data.while_statement.cons = deep_copy_expr_type_replace_worker(walk, source->data.while_statement.cons, relation, realias, rec_name, rec_type, 0);
		return new;
	case MATCH_EXPR:
		...
		new->data.match.pred = deep_copy_expr_type_replace_worker(walk, source->data.match.pred, relation, realias, rec_name, rec_type, 0);
		new->data.match.cases = pool_request(walk->parse->mem, sizeof(expr_ast)*new->data.match.count);
		for (uint64_t i = 0;i<new->data.match.count;++i){
			new->data.match.cases[i] = *deep_copy_expr_type_replace_worker(walk, &source->data.match.cases[i], relation, realias, rec_name, rec_type, 0);
		}
	case CAST_EXPR:
		new->data.cast.source = deep_copy_expr_type_replace_worker(walk, source->data.cast.source, relation, realias, rec_name, rec_type, 0);
	...
	case NOP_EXPR:
		return new;
	case STRUCT_ACCESS_EXPR:
		new->data.access.left = deep_copy_expr_type_replace_worker(walk, source->data.access.left, relation, realias, rec_name, rec_type, 0);
		new->data.access.right = deep_copy_expr_type_replace_worker(walk, source->data.access.right, relation, realias, rec_name, rec_type, 1);
		return new;
	case ARRAY_ACCESS_EXPR:
		new->data.access.left = deep_copy_expr_type_replace_worker(walk, source->data.access.left, relation, realias, rec_name, rec_type, 0);
		new->data.access.right = deep_copy_expr_type_replace_worker(walk, source->data.access.right, relation, realias, rec_name, rec_type, 0);
		return new;
	case FAT_PTR_EXPR:
		new->data.fat_ptr.left = deep_copy_expr_type_replace_worker(walk, source->data.fat_ptr.left, relation, realias, rec_name, rec_type, 0);
		new->data.fat_ptr.right = deep_copy_expr_type_replace_worker(walk, source->data.fat_ptr.right, relation, realias, rec_name, rec_type, 0);
		return new;
	}
	return new;
}
...
write_constant(genc* const generator, FILE* fd, const_ast* const constant){
	...
	fprintf(fd, " = ");
	write_expression(generator, fd, constant->value, 0, 0, 0, 0);
	fprintf(fd, ";\n");
}
...
write_term_impl(genc* const generator, FILE* fd, term_ast* const term){
	...
	write_expression(generator, fd, term->expression, 0, 1, 0, 0);
	fprintf(fd, "\n");
}
...
write_call(genc* const generator, FILE* fd, expr_ast* const expr, expr_ast* const first){
	if (expr->tag == APPL_EXPR){
		...
		write_expression(generator, fd, expr->data.appl.right, 0, 1, 0, 0);
		if (expr != first){
			fprintf(fd, ",");
		}
		return;
	}
	write_expression(generator, fd, expr, 0, 0, 0, 0);
	fprintf(fd, "(");
}

void
write_expression(genc* const generator, FILE* fd, expr_ast* const expr, uint64_t indent, uint8_t free, uint8_t access, uint8_t extern_access){
	switch (expr->tag){
	case APPL_EXPR:
		...
		if (first->tag == BINDING_EXPR){
			if (cstring_compare(&first->data.binding.data.name, "~add") == 0){
				fprintf(fd, "(");
				write_expression(generator, fd, prev_0->data.appl.right, 0, 1, 0, 0);
				fprintf(fd, ")+(");
				write_expression(generator, fd, prev_1->data.appl.right, 0, 1, 0, 0);
				fprintf(fd, ")");
				builtin = 1;
			}
		...
		break;
	case LAMBDA_EXPR:
		write_expression(generator, fd, expr->data.lambda.expression, indent, 1, 0, 0);
		break;
	case BLOCK_EXPR:
		ink_indent(fd, indent);
		fprintf(fd, "{\n");
		for (uint64_t i = 0;i<expr->data.block.line_count;++i){
			write_expression(generator, fd, &expr->data.block.lines[i], indent+1, 1, 0, 0);
			fprintf(fd, ";\n");
		}
		ink_indent(fd, indent);
		fprintf(fd, "}");
	...
	case TERM_EXPR:
		...
		if (expr->data.term->expression != NULL){
			fprintf(fd, " = ");
			write_expression(generator, fd, expr->data.term->expression, 0, 1, 0, 0);
		}
		break;
	case STRING_EXPR:
		ink_indent(fd, indent);
	...
	case LIST_EXPR:
		...
		for (uint64_t i = 0;i<expr->data.list.line_count;++i){
			...
			write_expression(generator, fd, &expr->data.list.lines[i], 0, 1, 0, 0);
		}
		fprintf(fd, "}");
		break;
	case STRUCT_EXPR:
			...
			if (expr->data.constructor.members[i].type != NULL){
				type_ast* fun_reduced = reduce_alias_and_type(generator->parse, expr->data.constructor.members[i].type);
			   	if (fun_reduced->tag == FUNCTION_TYPE){
					write_expression(generator, fd, &expr->data.constructor.members[i], 0, 0, 0, 0);
				}
				else{
					write_expression(generator, fd, &expr->data.constructor.members[i], 0, 1, 0, 0);
				}
			}
			else{
				write_expression(generator, fd, &expr->data.constructor.members[i], 0, 1, 0, 0);
	...
	case MUTATION_EXPR:
		ink_indent(fd, indent);
		write_expression(generator, fd, expr->data.mutation.left, 0, 1, 0, 0);
		fprintf(fd, " = ");
		write_expression(generator, fd, expr->data.mutation.right, 0, 1, 0, 0);
		break;
	case RETURN_EXPR:
		ink_indent(fd, indent);
		fprintf(fd, "return ");
		write_expression(generator, fd, expr->data.ret, 0, 1, 0, 0);
		break;
	case SIZEOF_EXPR:
		ink_indent(fd, indent);
		fprintf(fd, "sizeof(");
	...
	case REF_EXPR:
		ink_indent(fd, indent);
		fprintf(fd, "&(");
		write_expression(generator, fd, expr->data.ref, 0, 1, 0, 0);
		fprintf(fd, ")");
		break;
	case DEREF_EXPR:
		ink_indent(fd, indent);
		fprintf(fd, "*(");
		write_expression(generator, fd, expr->data.deref, 0, 1, 0, 0);
		fprintf(fd, ")");
		break;
	case IF_EXPR:
		ink_indent(fd, indent);
		fprintf(fd, "if (");
		write_expression(generator, fd, expr->data.if_statement.pred, 0, 1, 0, 0);
		fprintf(fd, ")");
		write_expression(generator, fd, expr->data.if_statement.cons, indent, 1, 0, 0);
		if (expr->data.if_statement.alt != NULL){
			fprintf(fd, "else\n");
			write_expression(generator, fd, expr->data.if_statement.alt, indent, 1, 0, 0);
		}
		break;
	case FOR_EXPR:
		ink_indent(fd, indent);
		fprintf(fd, "for (");
		write_expression(generator, fd, expr->data.for_statement.initial, 0, 1, 0, 0);
		fprintf(fd, ";");
		write_expression(generator, fd, expr->data.for_statement.limit, 0, 1, 0, 0);
		fprintf(fd, ";");
		write_expression(generator, fd, expr->data.for_statement.change, 0, 1, 0, 0);
		fprintf(fd, ")");
		write_expression(generator, fd, expr->data.for_statement.cons, indent, 1, 0, 0);
		break;
	case WHILE_EXPR:
		ink_indent(fd, indent);
		fprintf(fd, "while (");
	...
	case CAST_EXPR:
		...
		fprintf(fd, ")(");
		write_expression(generator, fd, expr->data.cast.source, 0, 1, 0, 0);
		fprintf(fd, ")");
		break;
	case BREAK_EXPR:
		ink_indent(fd, indent);
	...
	case STRUCT_ACCESS_EXPR:
		ink_indent(fd, indent);
		write_expression(generator, fd, expr->data.access.left, 0, 1, 0, 0);
		type_ast* reduced_left = reduce_alias_and_type(generator->parse, expr->data.access.left->type);
		if (reduced_left->tag == PTR_TYPE){
			fprintf(fd, "->");
			write_expression(generator, fd, expr->data.access.right, 0, 0, 1, 0);
		}
			...
			fprintf(fd, ".");
			if (reduced_left->tag == NAMED_TYPE){ // NOTE this is because reduce_alias_and_type stops at externs, so if its not a struct its an extern
				write_expression(generator, fd, expr->data.access.right, 0, 0, 1, 1);
			}
			else{
				write_expression(generator, fd, expr->data.access.right, 0, 0, 1, 0);
			}
	...
	case ARRAY_ACCESS_EXPR:
		ink_indent(fd, indent);
		write_expression(generator, fd, expr->data.access.left, indent, 1, 0, 0);
		if (expr->data.access.left->type->tag == FAT_PTR_TYPE){
			fprintf(fd, ".ptr");
		}
		fprintf(fd, "[");
		write_expression(generator, fd, &expr->data.access.right->data.list.lines[0], 0, 1, 0, 0);
		fprintf(fd, "]");
		break;
	case FAT_PTR_EXPR:
		ink_indent(fd, indent);
		...
		ink_indent(fd, indent+1);
		fprintf(fd, ".ptr=");
		write_expression(generator, fd, expr->data.fat_ptr.left, 0, 1, 0, 0);
		fprintf(fd, ",\n");
		ink_indent(fd, indent+1);
		fprintf(fd, ".len=");
		write_expression(generator, fd, expr->data.fat_ptr.right, 0, 1, 0, 0);
		ink_indent(fd, indent+1);
		fprintf(fd, "\n}");
		break;
	}
